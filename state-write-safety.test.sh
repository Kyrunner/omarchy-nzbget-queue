...
#
# Needs no NZBGet and no credentials -- it calls the state helpers directly.
#
...
STATE="$XDG_STATE_HOME/omarchy-nzbget"
...
import nzbget
nzbget._save_json(nzbget.ENDPOINT_FILE, {"which": "public"})
PY
...
import nzbget
print(nzbget._load_json(nzbget.ENDPOINT_FILE, "default"))
PY
)"
...
import nzbget
nzbget._save_json(nzbget.ENDPOINT_FILE, {"which": "public"})
print(nzbget._load_json(nzbget.ENDPOINT_FILE, {}).get("which"))
PY
)"
...
check "symlinked state root is not written through" \
  "absent" "$([ -e "$WORK/real/omarchy-nzbget/endpoint.json" ] && echo present || echo absent)"

mkdir -p "$WORK/real/omarchy-nzbget"; chmod 0700 "$WORK/real/omarchy-nzbget"
printf '{"which": "planted"}\n' > "$WORK/real/omarchy-nzbget/endpoint.json"
...
check "symlink higher in the path is refused" \
  "absent" "$([ -e "$WORK/up/realdir/state/omarchy-nzbget/endpoint.json" ] && echo present || echo absent)"
...
check "parent writable by others is refused" \
  "absent" "$([ -e "$WORK/open/state/omarchy-nzbget/endpoint.json" ] && echo present || echo absent)"
...
check "state dir created 0700" \
  "700" "$(stat -c %a "$WORK/home/.local/state/omarchy-nzbget" 2>/dev/null)"
